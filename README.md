...
- **Workout — bodyweight strength circuit** — pick a focus (Full body / Upper / Lower / Core), set 1–5 rounds, and run a timed circuit (30s work / 15s rest per move) with a live countdown ring, pause/skip controls, and a completion summary with an estimated calorie burn. No equipment needed — every exercise is bodyweight-only. Logged sessions get their own calendar (same day-browsing system as Food/Water/Sleep) and a dedicated reminder alarm with the same catch-up behavior as the others. The dashboard shows a simple "Done ✓ / Not yet" status for today.
...
- **Workout reminder** (Workout screen) — same daily-time pattern, so you can set a fixed time to train.
